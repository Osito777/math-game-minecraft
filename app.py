...
if st.session_state.get('boss_active', False):
    ...
    if st.button(f"GOLPE {b} ⚔️"):
        if ans == q["c"]: st.success("OK!"); st.session_state.score += 1
        else: st.error(f"Mal! {q['c']}")
